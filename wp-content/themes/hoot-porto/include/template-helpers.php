 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_branding' ) ):
function hoot_porto_branding() {
	?>
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_logo' ) ):
function hoot_porto_logo() {

	$display = '';
	$hoot_porto_logo = hoot_get_mod( 'logo' );
	...
	if ( 'text' == $hoot_porto_logo || 'custom' == $hoot_porto_logo ) {
		$display .= hoot_porto_get_text_logo( $hoot_porto_logo, $tag_h1, $tag_h2 );
	} elseif ( 'mixed' == $hoot_porto_logo || 'mixedcustom' == $hoot_porto_logo ) {
		$display .= hoot_porto_get_mixed_logo( $hoot_porto_logo, $tag_h1, $tag_h2 );
	} elseif ( 'image' == $hoot_porto_logo ) {
		$display .= hoot_porto_get_image_logo( $hoot_porto_logo, $tag_h1, $tag_h2 );
	}

	echo wp_kses( apply_filters( 'hoot_porto_logo', $display, $hoot_porto_logo, $tag_h1, $tag_h2 ), hoot_data( 'hootallowedtags' ) );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $hoot_porto_logo text|custom
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'hoot_porto_get_text_logo' ) ):
function hoot_porto_get_text_logo( $hoot_porto_logo, $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$title_icon = hoot_sanitize_fa( hoot_get_mod( 'site_title_icon', NULL ) );

	$class = $id = 'site-logo-' . esc_attr( $hoot_porto_logo );
	$class .= ( $title_icon ) ? ' site-logo-with-icon' : '';
	$class .= ( 'text' == $hoot_porto_logo && !function_exists( 'hoot_lib_premium_core' ) ) ? ' site-logo-text-' . hoot_get_mod( 'logo_size' ) : '';
	...
	$display .= '<div id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '">';
		...
		$display .= "<{$tag_h1} " . hoot_get_attr( 'site-title' ) . '>';
			$display .= '<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">';
				...
				$title = ( 'custom' == $hoot_porto_logo ) ? hoot_porto_get_custom_text_logo() : '<span class="blogname">' . get_bloginfo( 'name' ) . '</span>';
				$display .= apply_filters( 'hoot_porto_site_title', $title );
			$display .= "</a>";
		$display .= "</{$tag_h1}>";
	...
	return apply_filters( 'hoot_porto_get_text_logo', $display, $hoot_porto_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $hoot_porto_logo mixed|mixedcustom
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'hoot_porto_get_mixed_logo' ) ):
function hoot_porto_get_mixed_logo( $hoot_porto_logo, $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$has_logo = has_custom_logo();

	$class = $id = 'site-logo-' . esc_attr( $hoot_porto_logo );
	$class .= ( !empty( $has_logo ) ) ? ' site-logo-with-image' : '';
	$class .= ( 'mixed' == $hoot_porto_logo && !function_exists( 'hoot_lib_premium_core' ) ) ? ' site-logo-text-' . hoot_get_mod( 'logo_size' ) : '';
	...
	$display .= '<div id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '">';
		...
		$display .= '<div class="site-logo-mixed-text">';
			...
			$display .= "<{$tag_h1} " . hoot_get_attr( 'site-title' ) . '>';
				$display .= '<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">';
					$title = ( 'mixedcustom' == $hoot_porto_logo ) ? hoot_porto_get_custom_text_logo() : '<span class="blogname">' . get_bloginfo( 'name' ) . '</span>';
					$display .= apply_filters( 'hoot_porto_site_title', $title );
				$display .= "</a>";
			$display .= "</{$tag_h1}>";
	...
	return apply_filters( 'hoot_porto_get_mixed_logo', $display, $hoot_porto_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $hoot_porto_logo
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'hoot_porto_get_image_logo' ) ):
function hoot_porto_get_image_logo( $hoot_porto_logo = 'image', $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$has_logo = has_custom_logo();
	...
	return apply_filters( 'hoot_porto_get_image_logo', $display, $hoot_porto_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'hoot_porto_get_custom_text_logo' ) ):
function hoot_porto_get_custom_text_logo() {
	$title = '';
	$logo_custom = apply_filters( 'hoot_porto_logo_custom_text', hoot_sortlist( hoot_get_mod( 'logo_custom' ) ) );
		...
		if ( is_customize_preview() ) {
			...
			foreach ( $logo_custom as $logo_custom_line ) {
				...
				if ( !function_exists('hoot_lib_premium_core') && !$logo_custom_line['sortitem_hide'] && !empty( $logo_custom_line['font'] ) ) {
					...
					$css .= '#site-logo-custom .site-title-line' . $lcount . ',#site-logo-mixedcustom .site-title-line' . $lcount . '{font-style:italic}';
					}
				}
				$lcount++;
			}
			echo wp_kses( apply_filters( 'hoot_porto_customtext_selectiverefresh_inlinecss', '<style type="text/css">' . $css . '</style>' ), hoot_data( 'hootallowedtags' ) );
		}

	}
	return apply_filters( 'hoot_porto_get_custom_text_logo', $title, $logo_custom );
}
endif;
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_header_aside' ) ):
function hoot_porto_header_aside() {
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_secondary_menu' ) ):
function hoot_porto_secondary_menu( $context ) {
	$location = hoot_get_mod( 'menu_location' );
	$side = hoot_get_mod( 'logo_side' );
	if ( $location == 'side' ) { $location = 'none'; $side = 'menu'; }
 ...
 * @return void
 */
function hoot_porto_nav_menu_toplevel_items( $theme_location = 'hoot-primary-menu' ) {
 ...
 * @return string
 */
if ( !function_exists( 'hoot_porto_menu_description' ) ):
function hoot_porto_menu_description( $title, $item, $args, $depth ) {
	$return = '';
...
add_filter( 'nav_menu_item_title', 'hoot_porto_menu_description', 5, 4 );
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_add_custom_title_content' ) ):
function hoot_porto_add_custom_title_content( $location = 'pre', $context = '' ) {

	$pre_title_content_post = apply_filters( 'hoot_porto_pre_title_content_post', '', $location, $context );
	if ( ( $location == 'pre' && !$pre_title_content_post ) ||
		 ( $location == 'post' && $pre_title_content_post ) ) : 

		$pre_title_content = apply_filters( 'hoot_porto_pre_title_content', '', $location, $context );
		if ( !empty( $pre_title_content ) ) :

			$pre_title_content_stretch =  apply_filters( 'hoot_porto_pre_title_content_stretch', '', $location, $context ); ?>
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_display_404_content' ) ):
function hoot_porto_display_404_content() {
	echo esc_html( __( 'Apologies, but no entries were found.', 'hoot-porto' ) );
}
endif;
add_action( 'hoot_porto_404_content', 'hoot_porto_display_404_content', 5 );
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_footer_structure' ) ):
function hoot_porto_footer_structure() {
 ...
 * @return int
 */
function hoot_porto_get_footer_columns() {
	$footers = hoot_get_mod( 'footer' );
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_get_column_span' ) ):
function hoot_porto_get_column_span( $col_width ) {
	$return = array();
	switch( $col_width ):
		case '100':
...
/**
 * Wrapper function for hoot_porto_layout() to get the class names for current context.
 ...
 * @return string
 */
if ( !function_exists( 'hoot_porto_layout_class' ) ):
function hoot_porto_layout_class( $context ) {
	return hoot_porto_layout( $context, 'class' );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'hoot_porto_layout' ) ):
function hoot_porto_layout( $context, $return = 'size' ) {

	// Set layout if not already set
	$layout = hoot_data( 'currentlayout' );
	if ( empty( $layout ) )
		hoot_porto_set_layout();
 ...
 * @access public
 */
if ( !function_exists( 'hoot_porto_set_layout' ) ):
function hoot_porto_set_layout() {
	...
	$sidebar = esc_attr( apply_filters( 'hoot_porto_layout', $sidebar ) );

	// Save the layout for current view
	hoot_porto_set_layout_span( $sidebar );
 ...
 * @access public
 */
if ( !function_exists( 'hoot_porto_set_layout_span' ) ):
function hoot_porto_set_layout_span( $sidebar ) {
	$spans = apply_filters( 'hoot_porto_layout_spans', array(
		'none' => array(
			'content' => 9,
			'sidebar' => 0,
		),
 ...
 * @access public
 */
if ( !function_exists( 'hoot_porto_thumbnail_size_contentwidth' ) ):
function hoot_porto_thumbnail_size_contentwidth() {
	return 'span-' . hoot_porto_layout( 'content' );
}
endif;
add_filter( 'hoot_thumbnail_size_contentwidth', 'hoot_porto_thumbnail_size_contentwidth' );
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_frontpagearea_fontstyle_customizer' ) ):
function hoot_porto_frontpagearea_fontstyle_customizer() {
...
if ( is_customize_preview() ) add_action( 'wp_footer', 'hoot_porto_frontpagearea_fontstyle_customizer', 5 );
 ...
 * @access public
 */
if ( !function_exists( 'hoot_porto_titlearea_top' ) ):
function hoot_porto_titlearea_top() {
	...
	if ( in_array( 'no-sidebar', $full ) ) {
		$sidebar_size = hoot_porto_layout( 'primary-sidebar' );
		if ( empty( $sidebar_size ) )
			return apply_filters( 'hoot_porto_titlearea_top', true, 'no-sidebar', $full );
	}

	/* For Posts */
	if ( is_singular( 'post' ) ) {
		if ( in_array( 'posts', $full ) )
			return apply_filters( 'hoot_porto_titlearea_top', true, 'posts', $full );
		else
			return apply_filters( 'hoot_porto_titlearea_top', false, 'posts', $full );
	}

	/* For Pages */
	if ( is_page() ) {
		if ( in_array( 'pages', $full ) )
			return apply_filters( 'hoot_porto_titlearea_top', true, 'pages', $full );
		else
			return apply_filters( 'hoot_porto_titlearea_top', false, 'pages', $full );
	}

	/* Default */
	if ( in_array( 'default', $full ) )
		return apply_filters( 'hoot_porto_titlearea_top', true, 'default', $full );
	else
		return apply_filters( 'hoot_porto_titlearea_top', false, 'default', $full );
 ...
 * @access public
 */
if ( !function_exists( 'hoot_porto_loopmeta_header_img' ) ):
function hoot_porto_loopmeta_header_img( $context, $display ) {
	...
	$img_id = apply_filters( 'hoot_porto_loopmeta_header_img_id', $img_id, $context, $location, $view_id );
	$img_id = absint( $img_id );

	if ( ( $location == 'header' || $location == 'staticheader' || $location == 'staticheader-nocrop' ) && !empty( $img_id ) ) {
		$img_src = wp_get_attachment_image_src( $img_id, apply_filters( "hoot_porto_{$context}_imgsize", 'full', 'header' ) );
		$image = '';
		if ( !empty( $img_src[0] ) ) {
			if ( $location == 'header' ) {
				$wrap_attr = array(
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_ns_filter_avatar' ) ):
function hoot_porto_ns_filter_avatar( $avatar, $id_or_email, $size, $default, $alt, $args ) {
 ...
 * @return void
 */
if ( !function_exists( 'hoot_porto_post_prev_next_links' ) ):
function hoot_porto_post_prev_next_links() {
	...
	?><div id="loop-nav-wrap" class="loop-nav"<?php echo $style; ?>><?php
		previous_post_link( '<div class="prev">' . __( 'Previous Post: %link', 'hoot-porto' ) . '</div>', '%title' );
		next_post_link(     '<div class="next">' . __( 'Next Post: %link',     'hoot-porto' ) . '</div>', '%title' );
	?></div><!-- .loop-nav --><?php
	endif;
}
endif;
 ...
 * @access public
 */
if ( !function_exists( 'hoot_porto_jetpack_infinitescroll_render' ) ):
function hoot_porto_jetpack_infinitescroll_render(){
